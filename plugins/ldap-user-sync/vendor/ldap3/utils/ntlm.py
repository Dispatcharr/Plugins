...
class NtlmClient(object):
    ...
    def compute_nt_response(self):
        ...
        # NTLMv2 mandates HMAC-MD5 here (RFC-less MS-NLMP spec); this is a
        # keyed challenge-response, not password storage. The LDAP User
        # Sync plugin never performs NTLM bind - do_ntlm_bind() only
        # lazily imports this module when a caller explicitly requests it.
    ...
    def ntowf_v2(self):
        ...
        if len(passparts) == 2 and len(passparts[0]) == 32 and len(passparts[1]) == 32:
            ...
            try:
                # NTLMv2 mandates MD4 for the password hash (MS-NLMP spec);
                # this is a protocol-defined keyed digest, not password
                # storage. Never reached by this plugin - see note above.
                # codeql[py/weak-sensitive-data-hashing]
